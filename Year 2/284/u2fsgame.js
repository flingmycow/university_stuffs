...
function moveUserRobot()
{
	...
	if (end == true)
	{
		...
		{		
			...
			if ( (xCord >= 0 && xCord <= 9) && (yCord >= 0 && yCord <= 9) )
			{
...
{	
	...
	else
	{
		...
		if (highY == 10)
		{
			...
			}while ( (randomX < 0 || randomX > 9) && (randomY < 0 || randomY > 9) ); // If the generated values are numbers that are OUTSIDE of the grid, redo 
...
function endPlay()
{
	if (end == true)
	{
		...
		end = 0; // END MODE ON; should not respond to any more moves if the user puts any into the textfield
		displayFinal();
	}
}
